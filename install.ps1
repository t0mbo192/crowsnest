...
.DESCRIPTION
    ...
    Run from a git clone and the command runs crowsnest from that clone, so
    `git pull` updates it. Point -Exe at a downloaded crowsnest.exe instead and
...
# --- tshark, which crowsnest reads packets with -------------------------------
...
if ($tshark) {
    ...
    Write-Host ""
    Write-Host "tshark is required -- crowsnest reads packets with it."
    Write-Host ""
...
if ($Exe) {
    ...
    # Running from source: find a Python new enough to run crowsnest.
    $python = $null
    ...
    if (-not (Test-Path $entry)) { Fail "crowsnest.py not found next to this script." }

    # A .cmd shim rather than a copy, so `git pull` updates the command too.
    @"
@echo off
rem Generated by crowsnest install.ps1 -- runs crowsnest from its git clone.
"$python" "$entry" %*
...
Write-Host "  See interfaces      crowsnest interfaces"
Write-Host "  Watch live          crowsnest live -i Ethernet     (elevated terminal)"
Write-Host "  Read a capture      crowsnest read capture.pcapng"
if (-not $Exe) {
