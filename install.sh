...
#
# crowsnest runs from this git clone: it is pure Python, so there is nothing to
# compile and `git pull` is the whole update process. All this does is check the
# dependencies and put a `crowsnest` command on your PATH.
#
...
cat > "$TARGET" <<EOF
#!/usr/bin/env bash
# Generated by crowsnest install.sh -- runs crowsnest from its git clone.
